...
class SRenderY(nn.Module):
    def __init__(self, image_size, obj_filename, uv_size=256):
        ...
        face_uvcoords = face_vertices_(uvcoords, uvfaces)
        ...
        face_colors = face_vertices_(colors, faces)
    ...
    def forward(self, vertices, transformed_vertices, albedos, lights=None, light_type='point'):
        ...
        face_normals = face_vertices_(normals, self.faces.expand(batch_size, -1, -1))
        transformed_normals = vertex_normals(transformed_vertices, self.faces.expand(batch_size, -1, -1))
        transformed_face_normals = face_vertices_(transformed_normals, self.faces.expand(batch_size, -1, -1))
    ...
    def render_depth(self, transformed_vertices):
        ...
        z = z / z.max()
        # Attributes
        attributes = face_vertices_(z, self.faces.expand(batch_size, -1, -1))
        # rasterize
        rendering = self.rasterizer(transformed_vertices, self.faces.expand(batch_size, -1, -1), attributes)
    ...
    def render_normal(self, transformed_vertices, normals):
        ...
        # Attributes
        attributes = face_vertices_(normals, self.faces.expand(batch_size, -1, -1))
        # rasterize
        rendering = self.rasterizer(transformed_vertices, self.faces.expand(batch_size, -1, -1), attributes)
